...
{
    EVoiceRecognitionPropertyMap = 1,       // 地图
...
// 设置识别类型
- (void)setProperty:(TBDVoiceRecognitionProperty)property;
...
// mode请传入EVoiceRecognitionModeSearch或EVoiceRecognitionModeInput
// 从下一次识别开始生效
- (void)setServerURL:(NSString *)url withMode:(int)mode;

//- - - - - - - - - - - - - - - -功能设置- - - - - - - - - - - - - - - - - - - -
// 定制功能，目前支持自然语言解析（仅搜索模式）和输入模式返回结果格式定制
// 定制语义解析功能请传入key="nlu"，如果开启此功能，将返回带语义的json串，含义详见开发文档说明
